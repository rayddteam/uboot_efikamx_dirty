...
static int esdhc_initialize(bd_t *bis, uint32_t addr)
{
	struct fsl_esdhc *regs = (struct fsl_esdhc *)addr;
...
//	mmc->f_max = MIN(mxc_get_clock(MXC_ESDHC_CLK), 50000000);
	mmc->f_max = MIN(mxc_get_clock(MXC_IPG_PERCLK), 50000000);
...
{
	printf("%s: base = 0x%08x\n", __func__, cfg->esdhc_base);
	esdhc_initialize(bis, cfg->esdhc_base);
...
{
	return (-1);
	return esdhc_initialize(bis, MMC_SDHC1_BASE_ADDR) || esdhc_initialize(bis, MMC_SDHC2_BASE_ADDR);
}
